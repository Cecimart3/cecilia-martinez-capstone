...
class Grid extends Component {
  ...
  visualizeDijkstra() {
    ...
    this.setState(
      {
        searchedNodes: visitedNodesInOrder.length,
  ...
  clearGrid = () => {
    const grid = gridModel();
    this.setState({ grid, searchedNodes: 0, path: 0 }, () => {
  ...
  render() {
    ...
    return (
      <main className="main">
        ...
        {grid && <Results searched={searchedNodes} path={path} />}
        <Legend />
      </main>
    );
  }
  ...
  return nodes.map((node, index) => (
    <Node
      ...
      onMouseEnter={(row, column) =>
        mouseHandlers.handleMouseEnter(row, column)
      }
...
const createGrid = (gridModel, mouseHandlers) => {
  return gridModel.map((rowData, index) => (
    <div key={`${index}`}>{createColumns(rowData, mouseHandlers)}</div>
  ));
};
...
const gridModel = () =>
  Array(20)
    ...
    .map((row, rowIdx) =>
      Array(40)
        .fill(null)
        .map((column, colIdx) => createNode(rowIdx, colIdx))
    );
